...
class CT_opt():
    ...
    def CH_train(self,x,y,power):
        data =self. df[x+y].dropna()
        data = data[data[y[0]]>400]
        data = data[data[x[1]]>0]
        data = data[data[x[2]]>5]
        data = data.reset_index()
        xdata = data[x]
        ydata = data[y]
    ...
    def train(self):
        self.CT_cols  = ['CT_eff','Wet_bulb_temp','CT_Total_KW']
        self.CH_cols  = ['condenser_supply_temp','chiller_supply_temp','loading','chiller_RT','chiller_kwh']
        self.CWP_cols = ['condenser_supply_temp','condenser_temp_diff','CWP_Total_KW']
        self.CT_regr_kw,  self.CT_kw_df  = self.CT_train(self.CT_cols[:-1],[self.CT_cols[-1]],1)
        self.CH_regr_kw,  self.CH_kw_df  = self.CH_train(self.CH_cols[:-1],[self.CH_cols[-1]],0)
        self.CWP_regr_kw, self.CWP_kw_df = self.CWP_train(self.CWP_cols[:-1],[self.CWP_cols[-1]],2)
    ...
    def predict(self,c):
        ...
        self.CT_pred_kw = self.CT_regr_kw.predict(self.CT_sc.transform(np.stack((CT_eff,np.repeat(self.WB,100))).T))

        a = np.array([Tctws])
        b = np.repeat(np.array([self.df_last[['chiller_supply_temp','loading','chiller_RT']]]),100, axis=0)
        self.CH_pred_kw = self.CH_regr_kw.predict(self.CH_sc.transform(np.concatenate((a, b.T), axis=0).T))

        a = np.array([Tctws])
        b = np.repeat(np.array([self.df_last[['condenser_temp_diff']]]),100, axis=0)
        self.CWP_pred_kw = self.CWP_regr_kw.predict(self.CWP_sc.transform(np.concatenate((a, b.T), axis=0).T))
        self.Total_pred_kw = self.CH_pred_kw+self.CT_pred_kw+self.CWP_pred_kw
        d = np.diff(self.Total_pred_kw)
        self.best_index = np.argmin(np.where(d<-1,0,d))
