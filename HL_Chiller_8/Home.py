...
with tab[1]:
    ...
    st.markdown("***")
    st.plotly_chart(CH_line(df=df,ai_col='CH08_Set_Temp'), use_container_width=True)
    ...
    if max(plot_df.loads) > 90:
        AI_supply_temp = df.CH08_Set_Temp.dropna().iloc[-1] - 0.12
    else :
        AI_supply_temp = df.CH08_Set_Temp.dropna().iloc[-1] - 0.12
