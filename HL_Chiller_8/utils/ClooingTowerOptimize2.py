...
class CT_opt():
    ...
    def CH_train(self,x,y,power):
        data =self. df[x+y].dropna()
        data = data[data.CH_open_head==data.CH_open_head.iloc[-1]]
        data = data[data[x[2]]>0]
        data = data.reset_index()
        xdata = data[x]
        ydata = data[y]
    ...
    def train(self):
        self.CT_cols  = ['condenser_supply_temp','CT_eff','Approach','Wet_bulb_temp','condenser_temp_diff','chiller_supply_temp','loading','chiller_RT','CH_open_head','CT_Total_KW']
        self.CH_cols  = ['condenser_supply_temp','CT_eff','Approach','Wet_bulb_temp','condenser_temp_diff','chiller_supply_temp','loading','chiller_RT','CH_open_head','chiller_kwh']
        self.CWP_cols = ['condenser_supply_temp','CT_eff','Approach','Wet_bulb_temp','condenser_temp_diff','chiller_supply_temp','loading','chiller_RT','CH_open_head','CWP_Total_KW']
    ...
    def predict(self,c):
        ...
        b = np.repeat(np.array([self.df_last[['Wet_bulb_temp','condenser_temp_diff','chiller_supply_temp','loading','chiller_RT','CH_open_head']]]),100, axis=0)
        ...
        return round(self.best_Tctws,2)
